class LocationsController < ApplicationController
  ...
  def create
    ...
    if coordinates
      ...
      if forecast_data
        ...
        # Handle error: Forecast data could not be retrieved
        handle_error("Forecast data could not be retrieved. Please try again.", target: 'forecast_errors')
      end      
    else
      # Handle error: Location could not be geocoded
  ...
  def destroy
    ...
    respond_to do |format|
      format.turbo_stream do
        render turbo_stream: turbo_stream.remove("location_#{params[:id]}")
      end
  ...
  private
    def geocode_location(location_name)
      ...
      if response.success?
        ...
        # Assuming the API returns an array of locations and you've requested only the first one
        # Extract the latitude and longitude from the first location in the response
    ...
    def get_forecast_for(coordinates)
      response = HTTParty.get("https://api.open-meteo.com/v1/forecast", query: { latitude: coordinates[:lat], longitude: coordinates[:lon], daily: temperature_2m_max, temperature_2m_min })
